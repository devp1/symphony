...
3. Symphony owns the initial claim transition before worker launch and the final label transition after a verified handoff marker. A successful run produces a PR ready for review, comments concise evidence on the issue, writes `.symphony/handoff.json` with `state: "human-review"`, and lets Symphony run any configured evidence review plus autonomous PR review before moving the issue from `in-progress` to `human-review`. In local trusted mode, Symphony preserves a ready handoff marker across restart/recovery, verifies it before launching another turn, clears it after the tracker transition is verified, and parks this same Codex issue session at `human-review` so later `rework` can continue in the same thread. Rework must write a fresh marker after addressing feedback.
